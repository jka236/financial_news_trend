        ...
        p.apply(KafkaIO.<Long, String>read()
                ...
                .apply("ConvertToJson", MapElements.via(
                    ...
                    public Document apply(KV<String, Long> input) {
                        return Document.parse(String.format("{word : '%s', count: '%s', date: '%d'}",input.getKey(), 
                                        input.getValue(), numberOfDays));
                    }}))

                // .apply(MapElements.into(TypeDescriptors.strings())
                    // .via((KV<String, Long> wordCount) -> wordCount.getKey() + ": " + wordCount.getValue()))
                    // .via((KV<String, Long> wordCount) -> String.format("{word : %s, count: %s, date: %s}",wordCount.getKey(), wordCount.getValue(), stringDate)))
                ...
                .apply(MongoDbIO.write()
                    .withUri("mongodb+srv://rss_feed:rss_feed@cluster0.46sfz.mongodb.net/?retryWrites=true&w=majority")
